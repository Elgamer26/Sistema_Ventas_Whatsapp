...
{
    function registrar_clientes($nombress, $apellidoss, $numero_docu, $telefono_p, $correo_p, $direccions, $sexo, $password, $web)
    {
        try {
            $res = "";
            ...
            if (empty($data)) {
                ...
                if (empty($data_b)) {
                    $sql_a = "INSERT INTO cliente (nombres, apellidos, cedula, telefono, correo, direcion, sexo, ppassword, web) VALUES (?,?,?,?,?,?,?,?,?)";
    ...
    // CLIENTE DE TIENDA FISICA
    function listar_clientes()
    {
        try {
            $c = modelo_conexion::conexionPDO();
            $sql = "SELECT * FROM cliente WHERE web = 0";
            $query = $c->prepare($sql);
            $query->execute();
    ...
    // CLIENTE DE TIENDA WEB
    function listar_clientes_web()
    {
        try {
            $c = modelo_conexion::conexionPDO();
            $sql = "SELECT * FROM cliente WHERE web = 1";
            $query = $c->prepare($sql);
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
            $arreglo = array();
    ...
    function editando_cliente_clientes($id, $nombress, $apellidoss, $numero_docu, $telefono_p, $correo_p, $direccions, $sexo, $password)
    {
        try {
            $res = "";
            $c = modelo_conexion::conexionPDO();

...
class model_proveedor extends modelo_conexion
{
    function nuevo_proveedor($razons, $rucs, $telefono_p, $correo_p, $direccions, $descripcions, $encargados, $sexo)
    {
        try {
            ...
            $sql = "SELECT * FROM proveedor where razon_social = ?";
            $query = $c->prepare($sql);
            ...
            if (empty($data)) {
                $sql_b = "SELECT * FROM proveedor where ruc = ?";
                ...
                if (empty($data_b)) {
                    ...
                    $sql_b = "SELECT * FROM proveedor where correo = ?";
                    ...
                    if (empty($data_z)) {
                        $sql_a = "INSERT INTO proveedor (razon_social, ruc, telefono, correo, direccion, descripcions, encargado, sexo) VALUES (?,?,?,?,?,?,?,?)";
    ...
    function editar_proveedor($id, $razons, $rucs, $telefono_p, $correo_p, $direccions, $descripcions, $encargados, $sexo)
    {
        try {
            $res = "";
            $c = modelo_conexion::conexionPDO();
            $sql = "SELECT * FROM proveedor where razon_social = ? AND id_proveedor != ?";
            $query = $c->prepare($sql);
            $query->bindParam(1, $razons);
            ...
            if (empty($data)) {
                $sql_b = "SELECT * FROM proveedor where ruc = ? AND id_proveedor != ?";
                $query_b = $c->prepare($sql_b);
                $query_b->bindParam(1, $rucs);
                ...
                if (empty($data_b)) {
                    ...
                    $sql_b = "SELECT * FROM proveedor where correo = ? AND id_proveedor != ?";
                    $query_z = $c->prepare($sql_b);
                    $query_z->bindParam(1, $correo_p);
                    ...
                    if (empty($data_z)) {
                        $sql_a = "UPDATE proveedor SET razon_social = ?, ruc = ?, telefono = ?, correo = ?, direccion = ?, descripcions = ?, encargado = ?, sexo = ? WHERE id_proveedor = ?";
                        $querya = $c->prepare($sql_a);
                        $querya->bindParam(1, $razons);
                        $querya->bindParam(2, $rucs);
                        $querya->bindParam(3, $telefono_p);

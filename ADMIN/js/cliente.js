var funcion, tabla_cliente, tabla_clientes_web;
  ...
  $.ajax({
    ...
    if (resp > 0) {
      if (resp == 1) {
        ...
        cargar_contenido(
          "contenido_principal",
          "vista/cliente/nuevo_cliente.php"
        );
      } else if (resp == 2) {
        alerta = [
          "existe",
...
$("#tabla_clientes_").on("click", ".editar", function () {
  ...
  document.getElementById("direccions").value = data.direcion;
  document.getElementById("sexo").value = data.sexo;
  document.getElementById("password_clie").value = data.ppassword;
  document.getElementById("password_clie_no").value = data.ppassword;
...
function editar_clientes() {
  ...
  var password_c = document.getElementById("password_clie_c").value;

  var no = document.getElementById("password_clie_no").value;
  ...
  if(password != no){
    ...
    if(password != password_c){
      $("#password_obliga_c").html("Confirmar password");
      $("#password_obliga").html("Confirmar password");
      return swal.fire(
  ...
  alerta = ["datos", "Se esta editando el cliente", "Editando cliente"];
  mostar_loader_datos(alerta);
  ...
  $.ajax({
    ...
    if (resp > 0) {
      if (resp == 1) {
        document.getElementById("password_clie_c").value = "";
...
///////////// CLIENTE DE LA TIENDA WEB
function listar_clientes_web() {
  funcion = "listar_clientes_web";
  tabla_clientes_web = $("#tabla_clientes_web").DataTable({
    ordering: true,
    paging: true,
    aProcessing: true,
    aServerSide: true,
  ...
  //esto es para crearn un contador para la tabla este contador es automatico
  tabla_clientes_web.on("draw.dt", function () {
    var pageinfo = $("#tabla_clientes_web").DataTable().page.info();
    tabla_clientes_web
      .column(0, { page: "current" })
      .nodes()
      .each(function (cell, i) {
...
$("#tabla_clientes_web").on("click", ".inactivar", function () {
  //esto esta extrayendo los datos de la tabla el (data)
  var data = tabla_clientes_web.row($(this).parents("tr")).data(); //a que fila deteta que doy click
  //esta condicion es importante para el responsibe porque salda un error si no lo pongo
  if (tabla_clientes_web.row(this).child.isShown()) {
    //esto es cuando esta en tamaño responsibo
    var data = tabla_clientes_web.row(this).data();
  }
  var dato = 0;
  var id = data.id_cliente;
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      cambiar_estado_cliente_web(id, dato);
    }
  });
});

$("#tabla_clientes_web").on("click", ".activar", function () {
  //esto esta extrayendo los datos de la tabla el (data)
  var data = tabla_clientes_web.row($(this).parents("tr")).data(); //a que fila deteta que doy click
  //esta condicion es importante para el responsibe porque salda un error si no lo pongo
  if (tabla_clientes_web.row(this).child.isShown()) {
...
function cambiar_estado_cliente_web(id, dato) {
  var res = "";
  if (dato == 1) {
    res = "activo";
  } else {
  ...
  $.ajax({
    ...
    if (response > 0) {
      if (response == 1) {
        alerta = ["exito", "success", "EL estado se " + res + " con extio"];
        cerrar_loader_datos(alerta);
        tabla_clientes_web.ajax.reload();
      }
    } else {
      alerta = ["error", "error", "No se pudo cambiar el estado"];
      cerrar_loader_datos(alerta);
...
$("#tabla_clientes_web").on("click", ".editar", function () {
  //esto esta extrayendo los datos de la tabla el (data)
  var data = tabla_clientes_web.row($(this).parents("tr")).data(); //a que fila deteta que doy click
  //esta condicion es importante para el responsibe porque salda un error si no lo pongo
  if (tabla_clientes_web.row(this).child.isShown()) {
  ...
  $("#modal_editar_cliente_web").modal({ backdrop: "static", keyboard: false });
  $("#modal_editar_cliente_web").modal("show");
});

function editar_clientes_web() {
  var id = document.getElementById("id_cliente").value;
  ...
  $.ajax({
    ...
    if (resp > 0) {
      if (resp == 1) {
        document.getElementById("password_clie_c").value = "";
        $("#modal_editar_cliente_web").modal("hide");
        alerta = ["exito", "success", "El cliente se edito con exito"];
        cerrar_loader_datos(alerta);
        tabla_clientes_web.ajax.reload();
      } else if (resp == 2) {
